...
local config = {
    npc = "Dallheim",         -- player name we follow
...
Module.New("Keep Follow Friend", function ()
    ...
    if allowReach then
        ...
        -- load players
        for i, npc in ipairs(Creature.iNpcs(9, false)) do

            -- when player name is this same as we looking for.
            if string.lower(npc.name) == string.lower(config.npc) then
